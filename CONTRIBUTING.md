...
| Document | Contenu |
| --- | --- |
| [`spec/parite-ancien-format.md`](spec/parite-ancien-format.md) | 89 exigences : la parité avec les huit cartes de l'ancien format, et ce que le nouveau format publie que l'ancien ne pouvait pas connaître. Chaque exigence porte sa raison et une marque de provenance — `mesuré`, `catalogue`, `fonctionnel`, `décision`, `bloqué`. |
