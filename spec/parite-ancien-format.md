...
**Emplacement.** Ce document vit dans le dépôt des cartes, sous `spec/`, hors de `docs/` et donc hors du site — délibérément : une exclusion mkdocs tient jusqu'au jour où quelqu'un touche la configuration d'exclusion. Il est référencé depuis le guide de contribution, aux côtés du relevé de formes mesurées, parce que deux documents de contributeur invisibles ne servent personne.
...
| `catalogue` | Lu dans le **contrat publié** du module cible : catalogue d'entités et constructeurs d'attributs. | Fiable comme contrat. Non observé en fonctionnement. |
| `modèle` | Présent dans le **modèle interne** du module, et repris par aucun constructeur d'attributs : la donnée existe et n'atteint jamais la carte. | Inexploitable en l'état. C'est l'écart où vivent les quatre demandes de développement. |
...
| `bloqué` | **Verdict, non provenance.** Se lit dans la colonne de parité : l'exigence est écrite mais inapplicable en l'état. Sa provenance dit *pourquoi* — `modèle` quand la donnée existe sans être publiée, `catalogue` quand elle n'existe nulle part. | Fermé côté cartes, ouvert côté module. Chaque occurrence porte sa demande. |
...
| TRV-4 | **Exigence.** Aucune couleur littérale dans le code d'une carte ; toutes proviennent des variables de thème. *Échappatoire admise :* une couleur **fournie par le serveur** n'est pas une couleur écrite dans le code et ne tombe pas sous cette interdiction, **sous trois conditions**. *Une :* elle n'est acceptée qu'en **hexadécimal strict** — les noms CSS et `rgb()`, pourtant valides, sont refusés, parce qu'admettre une famille de syntaxes prend l'analyseur CSS entier pour frontière de confiance. C'est la seule valeur de serveur du projet qui atteigne un attribut de style : tout le reste est du texte, que le moteur de rendu échappe, et un point-virgule dans la valeur ajouterait une propriété arbitraire. *Deux :* elle sert d'**accent, jamais d'aplat de fond** — non par goût, mais parce que ces couleurs sont choisies pour un fond blanc et cassent le contraste dès qu'un thème sombre est actif, sans que le thème de l'utilisateur puisse le corriger. *Trois :* la refuser ne coûte que la couleur, la ligne s'affiche. *Raison :* les cartes doivent suivre le thème de l'utilisateur. Contre-exemple d'origine : un séparateur blanc en dur, invisible en thème clair. | `fonctionnel` |  à auditer |
...
| TRV-19 | **Exigence.** Aucune carte n'attend d'**évènement** sur un changement de couleur, de mémo ou de contenu de cours : ces trois champs sont **délibérément exclus** de la détection de changement du module. *Raison :* le producteur le dit et le motive — une correction de coquille par un professeur ne doit pas réveiller la maison. L'exclusion est un choix de conception, pas un oubli à signaler. *Corollaire :* la valeur reste néanmoins juste à l'affichage, les attributs étant reconstruits à chaque cycle depuis l'instantané de faits ; un changement de couleur en cours d'année apparaît donc à la collecte suivante, **silencieusement**. Une carte peut s'y fier pour afficher, jamais pour notifier. *Limite de la preuve :* le motif des constructeurs a été lu, la chaîne de rafraîchissement ne l'a pas été de bout en bout. | `catalogue` |  à auditer |
| TRV-20 | **Exigence.** Toute comparaison de professeurs ou de salles se fait sur des **ensembles**, jamais sur des séquences. *Raison :* le serveur amont réordonne ses listes entre deux réponses. Comparées en séquences, elles diffèrent sans que rien n'ait changé — mesuré côté module, où un cours co-enseigné émettait « cours modifié » chaque nuit. L'ordre d'une liste de professeurs ne porte aucune information et ne doit donc jamais en produire. *Portée :* préventive — aucune règle de ce document ne compare de telles listes aujourd'hui, et elle existe pour que la première qui le fera ne refasse pas l'erreur. | `catalogue` |  à auditer |
...
| EDT-27 | **Exigence.** L'**indicateur publié** dit *si* un cours a lieu ; les **horodatages** disent *lequel*. L'indicateur a donc un **droit de veto, jamais un droit de désignation** : sans lui, aucune mise en avant ; avec lui, c'est le cours dont le créneau contient l'instant. *Raison :* TRV-17 impose de lire le fait publié plutôt que de le recalculer, mais le fait publié est booléen et ne nomme aucun cours — ma rédaction précédente lui demandait de désigner, ce dont il est incapable. Une comparaison locale seule dérive entre deux poussées d'état ; l'indicateur seul ne peut pointer aucune ligne. Chacun répond à sa question et à une seule, faute de quoi deux sources finissent par se contredire. Voir CLO-9. | `catalogue` |  à auditer |
...
### Contrat d'élément — notes

| Clé | Type | Remarque |
| --- | --- | --- |
| `id` | chaîne | |
| `subject` | chaîne nullable | |
| **`value`** | nombre **nullable** | La valeur. Nulle quand la note est une mention. |
| `status` | chaîne nullable | La mention : absent, non noté, dispensé, non rendu, félicitations. **Du texte**, jamais un objet — le constructeur convertit l'énumération du modèle. |
| `out_of` | nombre nullable | Le barème. |
| `coefficient` | nombre nullable | |
| `date` | horodatage ISO | |
| `class_average` | nombre nullable | |
| `min` · `max` | nombres nullables | Bornes de classe. Renommées depuis le modèle. |
| `comment` | chaîne nullable | |
| `is_bonus` · `is_optional` | booléens | Sans équivalent d'origine. |

Trois champs du modèle ne sont pas publiés : l'identifiant de matière, le barème par défaut, et l'indicateur « ramené sur 20 ».

### Exigences

| Réf | Exigence | Prov. | Parité |
| --- | --- | --- | --- |
| NOT-1 | **Exigence.** La valeur d'une note est lue sous le nom `value`. *Raison :* confirmé dans les constructeurs. Le nom inventé qui a fait afficher **toutes** les notes comme absentes était `grade` — matière et coefficient corrects, valeur manquante, sans un signe d'erreur — et les tests ne l'ont pas vu parce que leurs fixtures reprenaient le nom inventé. Un nom deviné se vérifie contre lui-même. | `catalogue` |  à auditer |
| NOT-2 | **Exigence.** Une note peut n'être **pas un nombre** : la valeur est nulle quand la note est une mention, et le motif est porté par un champ de statut distinct, sous forme de texte. La carte rend la mention, jamais un zéro ni un tiret muet. *Raison :* garantie **par le code** et non par une mesure — l'état de la dernière note est dérivé de la valeur, si bien qu'un état numérique accompagné d'un statut non nul est *exclu par construction*. Un zéro affirmerait une note de zéro, ce qui est faux et lourd de conséquences. L'ancienne carte convertit la valeur en nombre sans condition. | `catalogue` |  à auditer |
| NOT-3 | **Exigence.** Le barème est lu et utilisé pour toute comparaison, et toute normalisation part de lui. *Raison :* le catalogue le déclare obligatoire pour comparer correctement — une note sur 10 et une note sur 20 ne se comparent pas. Le modèle porte un indicateur « ramené sur 20 » qui **n'est pas publié** : la normalisation ne peut donc venir que du barème. | `catalogue` |  à auditer |
...
### Contrat d'élément — moyennes par matière

| Clé | Type | Remarque |
| --- | --- | --- |
| **`subject_id`** | chaîne **nullable** | Ajouté par le module : l'objet d'amont ne porte aucun identifiant. Voir MOY-4. |
| `subject` | chaîne nullable | Le libellé. |
| `student` | nombre nullable | La moyenne de l'élève. |
| `class_average` | nombre nullable | |
| `min` · `max` | nombres nullables | Bornes de classe. **Absentes du bulletin.** |
| `out_of` | nombre nullable | |

La couleur de matière est dans le modèle et non publiée — un des porteurs de DEM-2.

### Contrat — bulletin

Pas de liste d'éléments homogène : trois attributs, dont `subjects` et `comments`. L'état porte le **nombre de matières**. Chaque matière :

| Clé | Type | Remarque |
| --- | --- | --- |
| `id` · **`name`** | chaînes | **`name`**, là où les moyennes disent `subject` : deux clés pour la même notion, à ne pas lisser. |
| `student_average` · `class_average` | nombres nullables | |
| `coefficient` | nombre nullable | **Absent des moyennes par matière.** |
| `comments` | tableau de chaînes | Les appréciations. **Absentes des moyennes.** |
| `teachers` | tableau de chaînes | **Absents des moyennes.** |

Ni `min` ni `max` : les bornes de classe n'existent que sur les moyennes par matière. La couleur et les bornes du modèle ne sont pas publiées.
...
| MOY-4 | **Exigence.** Les moyennes par matière sont indexées par l'identifiant de matière publié, **jamais par leur rang**, et la carte prévoit le cas où cet identifiant est nul — sans retomber sur le rang. *Raison :* confirmé par le code et par son commentaire — l'objet d'amont ne porte **aucun** identifiant, et celui-ci a été ajouté par le module précisément pour rendre l'indexation possible. Un index de position change dès qu'une matière apparaît ou disparaît, et associe alors la moyenne à la mauvaise matière. | `catalogue` |  à auditer |
| MOY-5 | **Exigence.** Les bornes de classe sont affichables séparément de la moyenne de classe. *Portée :* les moyennes par matière seulement — le bulletin n'en porte pas. | `catalogue` |  à auditer |
| MOY-6 | **Exigence.** Le bulletin est traité comme une famille distincte des moyennes par matière, et non comme leur variante. *Raison :* il porte ce que les moyennes n'ont pas — les **appréciations**, par matière et au niveau du bulletin, les **enseignants** par matière, et un **coefficient** de matière — et il ne porte **pas** les bornes de classe. Le libellé de matière s'y nomme de plus autrement. Aucune carte d'origine ne couvrait le bulletin ; c'est la famille la plus riche de la cible et la seule qui porte du texte d'enseignant. | `catalogue` |  à auditer |
| MOY-7 | **Exigence.** La moyenne de l'élève se lit sous le nom `student`, et non sous un nom déduit de la notion qu'elle porte. *Raison :* c'est le **troisième** incident du même mécanisme dans cette seule famille — une carte a lu `grade` au lieu de `value` et affichait toutes les notes en tiret ; une autre `average` au lieu de `student` ; un audit de contrat vient d'en trouver un troisième sur le masquage. Trois fois n'est plus de la malchance : les noms de cette famille décrivent des **rôles** — l'élève, la classe, le minimum — là où l'intuition attend la notion mesurée, et une clé devinée est donc systématiquement plausible et fausse. *Conséquence de méthode :* dans cette famille, aucune clé ne s'écrit sans le constructeur sous les yeux. | `catalogue` |  à auditer |
...
| --- | --- | --- |
| `id` | chaîne | |
| `name` | chaîne nullable | L'intitulé. |
| `subject` | chaîne nullable | |
| `date` | horodatage ISO | |
| `acquisitions` | tableau d'objets | Chacun : `name`, **`level`**, `abbreviation`, **`domain`**, tous nullables. |

**Cinq champs du modèle ne sont pas publiés** : l'enseignant, l'énoncé, l'identifiant de matière, le coefficient d'acquisition, son identifiant et son pilier de compétences. Un seul constructeur existe pour cette famille, vérifié. Voir EVA-10 et DEM-4.
...
| EVA-8 | **Exigence.** L'intitulé et la date d'une évaluation sont individuellement masquables. *Raison :* ce sont les deux seuls champs que le contrat publie parmi les cinq que l'exigence de parité réclamait. Voir EVA-10 pour les trois autres. | `catalogue` |  à auditer |
| EVA-9 | **Exigence.** Aucune évaluation : mention d'absence, sélecteur de période conservé. | `fonctionnel` |  à auditer |
| EVA-10 | **Exigence bloquée.** L'enseignant, l'énoncé et le coefficient d'une évaluation ne sont pas spécifiables. *Raison :* les trois existent dans le modèle interne et **aucun n'atteint la carte** — le constructeur de la famille, unique et vérifié, n'en reprend aucun. L'exigence de parité d'origine les déclarait masquables, ce qui aurait fait chercher une option incapable d'exister. Versés à DEM-4, avec l'identifiant d'acquisition et le **pilier** de compétences. | `modèle` | **bloqué** |
...
| CAN-5 | **Exigence bloquée** — requalifiée depuis « suspendue ». Les régimes et libellés par plat ne sont pas spécifiables en l'état. *L'ancien format les publiait, vérifié en source :* un plat y est un objet portant `name` et un tableau `labels`, chaque libellé portant son nom, et la carte les rend derrière une option dédiée (`src/cards/menus-card.js:39-46`). Ce n'est donc pas une exigence sans objet : c'est une **régression mesurable** entre les deux formats. *Où elle se produit :* la bibliothèque amont décode le libellé — identifiant, nom, couleur — et la passerelle du nouveau module **l'aplatit** dans une fonction dédiée qui ne retient que le nom : un plat entre comme objet et sort comme chaîne, le tableau `labels` est jeté là. Le relevé de la forme de menu en aval est exact et ne porte ni régime ni allergène — c'est précisément la conséquence. L'absence du champ dans la forme de menu du nouveau module est la **conséquence** de cet aplatissement, non la preuve que la donnée n'existe pas. Voir DEM-3 et CLO-8. *Gravité :* seul endroit du projet où l'information manquante a des conséquences hors de l'écran — un allergène non affiché. | `mesuré` | **bloqué** — publié par l'ancien format, aplati par le nouveau |
...
| CAN-9 | **Exigence.** Aucun indicateur de dîner n'est **affirmé**. *Raison :* le champ existe dans le modèle amont et n'est pas publié ; l'inférer du seul drapeau de déjeuner traiterait son absence de valeur comme une négation, ce que CAN-2 interdit. *Nuance apportée par CAN-11 :* la carte peut néanmoins dire qu'un service **n'est pas** un déjeuner, puisque ce drapeau-là est publié. Nier est permis, nommer ne l'est pas. | `catalogue` |  à auditer |
...
| CAN-11 | **Exigence.** La carte n'annonce un menu comme celui du **déjeuner** que si le drapeau publié le dit. *Raison :* le capteur « menu du jour » sert le déjeuner s'il existe, **et sinon n'importe quel repas de la journée** — le repli est explicite dans le producteur. Un établissement qui publie un dîner sans déjeuner alimente donc ce capteur avec le dîner, et une carte qui titre « midi » énonce alors quelque chose de **faux**. Le drapeau qui le dirait est publié et n'est lu par aucune carte. *Trois états, pas deux :* vrai — « déjeuner » ; faux — « ce n'est pas le déjeuner », sans nommer le service (CAN-9) ; **nul** — le producteur reste muet plutôt que de deviner, et la carte se tait de même. *Classement :* ce n'est pas une fonctionnalité manquante mais une **affirmation potentiellement fausse**, la catégorie prioritaire. | `catalogue` | **manquant** — champ publié, lu par aucune carte |
...
| DEM-2 | **La couleur de matière dans un attribut.** Décodée **jusque dans l'objet de transfert** — **trois** classes en portent une, non quatre : le créneau, le devoir et la moyenne. Les **quatre** sont des *sites de décodage* dans la passerelle, dont deux construisent le même créneau par deux chemins. Trois objets, quatre sites, trois familles côté carte — le mot « objets » portait l'erreur, et les deux chiffres se réconcilient sur « sites » — et seulement non republiée ; explicitement exclue de la comparaison de changements, donc connue et volontairement retenue. Même situation pour l'identifiant de matière, les **groupes**, les classes virtuelles, le numéro, le lieu et la durée. **Coût :** un constructeur d'attributs à compléter, aucune requête réseau, aucune rupture de contrat. | EDT-D2 reste bloquée, et EDT-D1 reste sans l'information de groupe qui l'expliquerait. |
| DEM-3 | **Les libellés de plat, et le nom du repas.** Trois champs de la même famille, à ne pas confondre avec DEM-2. Le libellé par plat — porteur d'un régime ou d'un allergène, et d'une couleur — est décodé par la bibliothèque amont puis **jeté par la passerelle**, avant tout objet de transfert. L'exposer demande donc un nouvel objet, un changement de type sur les six champs de repas et **une rupture de contrat sur les six attributs que la section 6 vient de spécifier**, ainsi que sur les cartes qui les lisent. **Deux moitiés de coût très inégal, à ne pas traiter ensemble.** Le nom du repas **et le drapeau de dîner** existent dans l'objet de transfert et ne sont publiés nulle part : même situation que DEM-2, coût quasi nul — et le drapeau de dîner est celui qui compléterait CAN-9 et CAN-11. Le libellé par plat, lui, est la moitié chère décrite ci-dessus. **Aucun de ces champs ne coûte de requête** — ils arrivent dans la même réponse que les noms de plats, déjà décodés. Le coût est entièrement dans le modèle et le contrat, nul sur le réseau. *Bénéfice secondaire :* le libellé portant une couleur **fournie par le serveur**, la carte cantine gagnerait une couleur admise par l'échappatoire de TRV-4. | CAN-5 reste suspendue, et c'est le seul manque du projet dont les conséquences sortent de l'écran. |
| DEM-4 | **Les champs d'évaluation retenus dans le modèle.** L'enseignant, l'énoncé et l'identifiant de matière d'une évaluation, plus le coefficient, l'identifiant et le **pilier** de compétences d'une acquisition — un niveau de regroupement au-dessus du domaine. Tous dans le modèle, repris par aucun constructeur d'attributs. **Même classe de coût que DEM-2** : un constructeur à compléter, aucune rupture de contrat, aucune requête réseau. À grouper avec elle si vous les traitez ensemble. | EVA-10 reste bloquée et EVA-8 est réduite à deux champs sur les cinq que la parité réclamait. |
...
| CLO-6 | **Les libellés de plat.** Posés comme question ouverte — un champ décodé et non publié, comme la couleur ? **Réponse : plus grave.** Décodé en amont, puis jeté à la frontière de la passerelle, avant tout objet de transfert. Présenter cette demande comme équivalente à DEM-2 aurait fait perdre en crédibilité sur celle qui est facile ; les deux sont désormais séparées et ordonnées. | tranché sur lecture de la chaîne |
| CLO-7 | **EVA-8 réclamait le masquage de champs inexistants.** Portée telle quelle depuis l'ancienne carte, elle déclarait cinq champs masquables dont **trois n'atteignent jamais la carte**. Détectée en confrontant l'exigence aux constructeurs et non aux objets du modèle — l'écart entre les deux est précisément là où vivent les quatre demandes de développement du projet. Scindée en EVA-8 tenable et EVA-10 bloquée. | corrigé sur lecture des constructeurs |
| CLO-8 | **CAN-5 contestée comme « sans objet », et confirmée.** L'argument : la forme de menu du nouveau module ne porte aucun champ de régime, donc la donnée n'existerait nulle part dans la chaîne, et l'ancienne carte affichait probablement le nom du repas. **Les deux lectures portaient sur des couches différentes** — l'une la forme du nouveau module, l'autre la frontière bibliothèque/passerelle — et elles sont compatibles : l'absence en aval est produite par l'aplatissement en amont. Tranché en lisant l'ancienne carte, qui parcourt un tableau `labels` par plat : la fonctionnalité existait. *Rétractée par son auteur, avec un diagnostic plus utile que l'erreur :* la lecture était bonne, c'est la **portée** donnée à la conclusion qui était fausse — conclure sur la chaîne entière depuis une couche. La preuve contraire a d'ailleurs été lue quelques minutes plus tard sans que le message parti soit corrigé, ce qui est le défaut réel. *Règle adoptée des deux côtés :* dire quel fichier et quelles lignes on a ouverts, **et ce qu'on n'a pas ouvert**. | confirmé sur lecture de l'ancien code, rétractation reçue |
| CLO-9 | **EDT-27 demandait à un booléen de désigner un cours.** Écrite pour appliquer TRV-17, elle confiait à l'indicateur publié le choix du cours à mettre en avant — or il dit seulement qu'un cours a lieu. Un implanteur l'aurait suivie sans trouver aucun moyen de l'appliquer. Le partage est désormais écrit : veto à l'indicateur, désignation aux horodatages. *Leçon générale :* « lire le fait publié » n'a de sens que si le fait publié répond à la question posée. | corrigé sur remarque d'implantation |
