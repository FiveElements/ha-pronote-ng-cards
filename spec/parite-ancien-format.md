...
| CAN-5 | **Exigence suspendue.** Les régimes et libellés par plat ne sont pas spécifiables en l'état. *Raison, désormais précise :* la bibliothèque amont décode bel et bien un libellé par plat — un objet portant identifiant, nom et **couleur** — mais la passerelle du module **le jette** : un plat y devient une simple chaîne, son nom. Le libellé n'atteint donc aucun objet de transfert. Ce n'est pas un champ non publié, c'est un champ **perdu plus tôt**, et la distinction change entièrement le coût de la demande : voir DEM-3. *Gravité :* c'est le seul endroit de tout ce projet où l'information manquante a des conséquences hors de l'écran — un allergène non affiché. | `catalogue` | **invérifiable** — perdu en amont du modèle |
